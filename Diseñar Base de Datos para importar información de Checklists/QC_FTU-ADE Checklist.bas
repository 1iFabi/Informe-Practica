...
Public Porcentaje_Falla_ADE As Double
Public Porcentaje_Falla_FDA As Double
...
RF_Nombre = HojaTapa.Cells(4, 1)
Version = HojaTapa.Cells(6, 1)
Nombre_Checklist = HojaTapa.Cells(1, 1)
...
ElseIf Trim(Version) = "" Or Trim(Version) = "Versión Documento <<X.X>>" Then
    MsgBox "No se ha ingresado Versión del Documento (Hoja Tapa)", vbInformation
...
Dim FilaInicioFTU As Integer
Dim FilaFinFTU As Integer
'--- FILAS DE CONCEPTOS FTU
FilaInicioFTU = 4
FilaFinFTU = 20

Dim FilaInicioADE As Integer
Dim FilaFinADE As Integer
'--- FILAS DE CONCEPTOS FTU
FilaInicioADE = 23
FilaFinADE = 34
...
Porcentaje_Falla_FTU = HojaMetrica.Cells(2, 6)
Porcentaje_Falla_ADE = HojaMetrica.Cells(3, 6)
Porcentaje_Eval = HojaMetrica.Cells(7, 6)
...
Else
    ...
    "'" & Replace(Revisor, "'", "''") & "', " & _
    "'" & Replace(Area, "'", "''") & "'); SELECT SCOPE_IDENTITY() As NewID;" 'ESTO ES RARO
...
'--- INSERTAR VALORES EN TABLA METRICAS
    ...
    SQL = "SET NOCOUNT ON; INSERT INTO dbo.Metricas (porcentaje_falla_ade, porcentaje_falla_ftu, porcentaje_eval, id_datos) VALUES (" & _
CStr(Replace(CStr(Porcentaje_Falla_FTU), ",", ".")) & ", " & _
CStr(Replace(CStr(Porcentaje_Falla_ADE), ",", ".")) & ", " & _
...
'--- INSERTAR VALORES EN TABLA FTU

For i = FilaInicioFTU To FilaFinFTU
    ...
    SQL = "INSERT INTO dbo.Evaluacion_FTU (conceptos_eval, id_valor, id_metricas) VALUES (" & _
...
'--- INSERTAR VALORES EN TABLA ADE
For f = FilaInicioADE To FilaFinADE
        Conceptos_Eval = Trim(HojaCiclo.Cells(f, 1))
        estadoCategoria = Trim(LCase(HojaCiclo.Cells(f, 4).Value))
    
        Categoria = ConvertirCategoria(estadoCategoria)
        
    SQL = "INSERT INTO dbo.Evaluacion_ADE (conceptos_eval, id_valor, id_metricas) VALUES (" & _
"'" & Replace(Conceptos_Eval, "'", "''") & "', " & _
CStr(Categoria) & ", " & _
CStr(idmetricas) & ")"
